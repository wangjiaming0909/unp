...
std::vector<poll_event_repo::event_tuple>::const_iterator poll_event_repo::find(Event_Type type) const
{
  ...
  return std::find_if(types_and_handlers_.begin(), types_and_handlers_.end(),
      [type] (const event_tuple& tuple) {
        if((tuple.event == EventHandler::READ_EVENT || tuple.event == EventHandler::ACCEPT_EVENT) && type == POLLIN)
          return true;
        if((tuple.event == EventHandler::WRITE_EVENT || tuple.event == EventHandler::CONNECT_EVENT) && type == POLLOUT)
          return true;
        if((tuple.event == EventHandler::EXCEPT_EVENT && type == POLLPRI))
          return true;
        return false;
      });
}
