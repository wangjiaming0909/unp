...
net::sock_acceptor::sock_acceptor(
    const inet_addr& local_addr, int reuse_addr,
    int protocol_family, int backlog,
    int protocol, sock_type type)
...
int net::sock_acceptor::open(){
  ...
  //then bind, listen
  return this->shared_open(local_addr_, protocol_family_, backlog_);
}

int net::sock_acceptor::close(){
  if(sock_fd_) return sock_fd_->close(-1);
  return 0;
}

int net::sock_acceptor::accept(
    SockStream& client_stream, inet_addr* remote_addr,
    microseconds timeout, bool restart) const
{
  //non-blocking accept
  ...
  for(;;){
    ...
    int client_fd = ::accept(sock_fd_->get_fd(), &addr, len_ptr);
...
int net::sock_acceptor::shared_open(
  ...
  ret = ::listen(sock_fd_->get_fd(), backlog);
  if(ret != 0) {
...
int net::sock_acceptor::shared_accept_start(microseconds timeout, bool restart) const
{
  ...
  return unp::handle_timed_accept_using_poll(sock_fd_->get_fd(), &timeout_milliseconds, restart);
