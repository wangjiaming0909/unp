...
TEST(buffer, test_remove)
{
  ...
  {
    buffer bb;
    bb.append(SizableClass<1023>());
    ASSERT_EQ(bb.total_len(), 1023);
    ASSERT_EQ(bb.chain_number(), 1);
    bb.drain(1023);
    ASSERT_EQ(bb.total_len(), 0);
    ASSERT_EQ(bb.last_chain_with_data(), nullptr);
  }
}
...
TEST(buffer, realloc)
{
  buffer_chain b1, b2, b3;
  SizableClass_WithChar<4096> c1;
  SizableClass_WithChar<4096> c2;
  SizableClass_WithChar<4096> c3;

  b1.append(c1.buffer_, 4000);
  b2.append(c2.buffer_, 4000);
  b3.append(c3.buffer_, 4000);
  auto ret = b1.append(b2);
  ASSERT_EQ(b1.size(), 4000+4000);
  ASSERT_EQ(ret, 4000);

  ret = b2.append(c1.buffer_, 200);
  ASSERT_EQ(ret, 200);
  ASSERT_EQ(b2.size(), 4200);
}

TEST(buffer, move)
{
  buffer_chain c1, c2;
  uint32_t default_size = buffer_chain::DEFAULT_CHAIN_SIZE;
  ASSERT_EQ(c1.chain_capacity(), default_size);
  ASSERT_EQ(c2.chain_capacity(), default_size);
  ASSERT_EQ(c1.size(), 0);
  ASSERT_EQ(c2.size(), 0);

  SizableClass_WithChar<4096> s1, s2;

  c1.append(s1.buffer_, 2000);
  c2.append(s2.buffer_, 3000);
  ASSERT_EQ(c1.size(), 2000);
  ASSERT_EQ(c2.size(), 3000);

  //actually append with left value reference
  c1.append(std::move(c2));
  ASSERT_EQ(c1.size(), 5000);

  buffer b;
  b.append(std::move(c1));

  ASSERT_EQ(c1.size(), 0);
  ASSERT_EQ(c1.get_buffer(), nullptr);
  ASSERT_EQ(b.total_len(), 5000);
  ASSERT_EQ(b.chain_number(), 1);
}

TEST(buffer, sync_client)
{
  buffer b;
  for(int i = 0; i < 1000; i++) {
    buffer_chain c;
    SizableClass_WithChar<29> s;
    c.append(s.buffer_, 29);
    b.append(std::move(c));
  }
